...
class CassandraFetchLogEntryDAO(ttl: CassandraColumnTTL)(implicit session: Session) extends FetchLogEntryDAO {
  ...
  private[dao] def bindFromEntity(bs: BoundStatement, fle: FetchLogEntry): BoundStatement = bs.bind()
  ...
  def find(jobId: String): Seq[FetchLogEntry] =
    mapAll(session.execute(stmtFindByPk.bind().setString("job_id", jobId))) {
      row => rowToEntity(row)
    }

  private[dao] def rowToEntity(row: Row): FetchLogEntry = FetchLogEntry(
