 ...
 */
class DefaultRobotRulesCache(val httpClient: HttpClient)
      (implicit val execContext: ExecutionContext) extends RobotRulesCache {
  ...
  def allow(userAgent: String, uris: Seq[CrawlUri]): Future[Seq[(CrawlUri, Boolean)]] =
    Future.sequence(
      uris.map({ uri =>
        val r = uri.reader
        getSiteEntry(userAgent, r.schemeToPort) map (
